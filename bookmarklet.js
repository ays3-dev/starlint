(function(){
    ...
    if(panel){
        panel.remove();
        var ss=document.getElementById(SNAP_ID); if(ss)ss.remove();
        var gs=document.getElementById(GHOST_ID); if(gs)gs.remove();
        document.querySelectorAll('[class*=__sl_s_]').forEach(function(el){
            Array.from(el.classList).filter(function(c){return c.startsWith('__sl_s_');}).forEach(function(c){el.classList.remove(c);});
        });
        return;
    }
    ...
    allEls.forEach(function(el,i){
      ...
      if(bad){
          var cls='__sl_s_'+i; el.classList.add(cls);
          snapCSS+='.'+cls+'{outline:2px solid #ff4757!important;outline-offset:1px;}';
          spIssues.push({tag:el.tagName.toLowerCase(),id:el.id,cls:(el.className+'').split(' ').filter(function(c){return c&&!c.startsWith('__sl_');}).slice(0,2).join(' ')});
      }
    });
    ...
    document.querySelectorAll('input,textarea,select').forEach(function(inp){
        ...
        if(!hasLabel&&!hasAria)a11yIssues.push({type:'Missing label',el:'<'+inp.tagName.toLowerCase()+' type="'+(inp.getAttribute('type')||'')+'">'});
    });
